...
记法：我们通常写 $a R b$ 表示 $(a, b) \in R$。也可以将 $R$ 视为谓词，写 $R(a, b)$ 表示 $(a, b) \in R$。当 $(a, b) \in R$ 时，我们说 $a$ 在 $R$ 中关联到 $b$，但注意方向是重要的——我们将 $(a, b) \in R$ 画成从 $a \in A$ 到 $b \in B$ 的有向箭头。

顺便一提，二元关系就是关系数据库中的"关系"！它只是数据对，有些对存在，有些不存在。
...
计数！不是数 1, 2, 3, ...，而是求集合的大小。有多少种洗牌方式？答案：$52!$。有多少棵节点为 $\{1, 2, \ldots, n\}$ 的树？令人惊讶的答案：$n^{n-2}$。
